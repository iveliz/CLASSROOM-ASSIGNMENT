...
export default function ResetPassword({ token, email }: Props) {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (
      ...
      axios
        .put(`${endpoint}/recuperarContraseña/contraseña`, {
          nombre_usuario: form.data.user_name,
          contraseña: form.data.password,
        })
        .then(response => {
          if (response.data == 1) {
            ...
            handleCloseBack();
            alert(
              'Ocurrio un problema revisa los datos que enviaste y vuelve a intentarlo',
            );
          }
        });
      handleOpenBack();
  ...
  const controlarError = (error: any, pass: any, confPass: any) => {
    ...
    if (error == 1 && pass.length < 8) {
      ...
      res =
        confPass.length == 0 && pass.length == 0
          ? ''
          : 'La confirmación no es igual a la contraseña nueva';
        ...
        <div className="mt-8">
          ...
          <JetInput
            ...
            onChange={e => {
              ...
              setErrorDos(
                controlarError(2, form.data.password, e.currentTarget.value),
              );
            }}
            autoComplete="new-password"
          />
        ...
        >
          <Box sx={{ ...style, width: 500 }} className="">
            <div id="child-modal-description">
              <p className="mx-8 my-8 align-middle">
                La contraseña fue restablecida con éxito, será redireccionado
                para iniciar sesión
              </p>
            </div>
