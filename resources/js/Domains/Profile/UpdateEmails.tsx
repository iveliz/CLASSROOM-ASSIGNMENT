...
export default function UpdateEmails({ user }: Props) {
  ...
  const getCorreos = async () => {
    await axios
      .post(`${endpoint}/correoElectronico/mostrar`, { id_usuario: user.id })
      .then(response => {
        setPrincipal(
          response.data[0].email_principal != null
            ? response.data[0].email_principal
            : '',
        );
        setSecundario(
          response.data[0].email_secundario != null
            ? response.data[0].email_secundario
            : '',
        );
        setCorreos({
          email_principal:
  ...
  return (
          ...
          <button
            ...
            onClick={() => {
              if (erroPrincipal == '' && erroSecundario == '') {
                if (
                  principal == correos.email_principal &&
                  secundario == correos.email_secundario
                ) {
                  alert(
                    'No se realizo ningun cambio, no hay una actualizacion valida',
                  );
                } else {
                  if (
                    principal != correos.email_principal &&
                    ...
                    if (principal != correos.email_principal) {
                      setMensajeModal(
                        'Esta seguro de editar el email principal',
                      );
                    } else if (secundario != correos.email_secundario) {
                      setMensajeModal(
                        'Esta seguro de editar el email secundario',
                      );
                    }
                  }
                  openModal();
    ...
    >
      ...
      <div className="col-span-6 sm:col-span-4">
        ...
        <JetInput
          ...
          onChange={e => {
            ...
            if (
              e.currentTarget.value == secundario &&
              ...
              if (
                e.currentTarget.value.length != secundario.length &&
                secundario.length > 0 &&
                e.currentTarget.value.length > 0 &&
                secundario.length <= 50 &&
        ...
        <p className="absolute text-sm text-red-600 mt-2 mb-0">
          {erroPrincipal}
        </p>
      </div>
      {/* <!-- Email secundario --> */}
      ...
      <Modal
        ...
        ariaHideApp={false}
      >
        <div className="font-bold">{mensajeModal}</div>
